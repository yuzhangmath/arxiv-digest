...
def _identity(path: Path, *, directory: bool = False) -> os.stat_result:
    info = path.lstat()
    expected_type = stat.S_ISDIR if directory else stat.S_ISREG
    if (
        not expected_type(info.st_mode) or info.st_uid not in {0, os.getuid()}
        or info.st_mode & 0o7000
        or (not directory and (info.st_nlink != 1 or not info.st_mode & 0o111))
    ):
...
def secure_interpreter(base: Path, cache: Path, *, platform: str) -> Path:
    ...
    if (
        ...
        or not any(base != root and base.is_relative_to(root) for root in roots)
    ):
        raise CiPythonError("selected Python is outside the hosted toolchain")
    # Validate both before changing either. The detector also verifies the
    # original interpreter's bin directory after nested pipx venv creation.
    selected = ((base, False, "interpreter"), (base.parent, True, "interpreter parent"))
    before = [_identity(path, directory=directory) for path, directory, _ in selected]
    for (path, directory, label), old in zip(selected, before):
        _report(f"{label} before", old)
        if old.st_mode & 0o022:
            subprocess.run(("sudo", "chmod", "go-w", str(path)), check=True)
        current = _identity(path, directory=directory)
        if (
