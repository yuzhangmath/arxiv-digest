...
def test_dashboard_views_are_accessible_and_responsive(width: int) -> None:
    with running_dashboard() as (server, _fixture), browser_page(
        ...
        assert page.get_by_text(
            "Historical coverage backfill: interrupted", exact=False
        ).is_visible()
        assert page.get_by_text(
            "Metadata synchronized through 2026-08-21", exact=True
        ).is_visible()
        assert page.get_by_text(
            "math.AG synchronization failed", exact=False
        ).is_visible()
        assert page.get_by_text("all categories synchronized").count() == 0
        ...
        open_folder = page.get_by_role("button", name="Open folder")
        open_folder.focus()
...
def test_backup_folder_cache_and_failed_restore_keep_the_session_usable() -> None:
    with running_dashboard() as (server, fixture), browser_page(
        ...
        page.locator('#settings input[value="documents"]').check()
        ...
        page.locator('#settings input[value="picker_87654321"]').check()
        with page.expect_response(
            lambda response: response.url.endswith("/settings/folder/test")
        ):
            page.get_by_role("button", name="Test download").click()
        ...
        page.get_by_role("button", name="Delete cache").click()
        page.get_by_role("button", name="Confirm delete cache").click()
        ...
        page.locator('#restore input[value="documents"]').check()
        page.locator(
        ...
        assert page.get_by_role("button", name="Open folder").is_enabled()
        ...
        assert all(
            payload["destination_choice"] == "documents"
